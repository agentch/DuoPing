...
v0.1 面向单台 Windows 电脑上的单个 Duolingo 账号，以用户设置的每日 XP 为完成标准。应用只读取学习数据，不提交课程、练习或社交操作。每日任务进度作为 v0.1 后的第一项增量功能开发；历史趋势、连胜预警、排行榜、云同步、自动更新和其他桌面平台仍不在当前范围内。
...
| M4.1 | 任务进度 | 首页分区展示当天任务、好友任务、本月特别任务及其单项进度与完成状态；手动和计划刷新均可更新 |
